...
| Off-main-thread | Web Worker | whole pipeline in a worker; stream progress. |
...
|---|---|---|
| **0 — Scaffold** | bun SPA + worker; ORT Web reports provider; COOP/COEP true; PDF→canvas display. | toolchain, isolation, WebGPU path. |
| **1 — Segmentation** | oemer UNets running; masks overlaid on page. | model opset/layout/quantization, tiling, memory, perf. **Biggest infra risk — front-loaded.** |
