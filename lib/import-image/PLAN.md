...
| **5 — Grand staff** | ~~paired staves, two-staff part, per-staff voices~~ (done — `groupSystems` pairs treble/bass by clef, `buildScore` emits `<staff>`/`<backup>`/per-staff voices). **Remaining:** brace detection from the image (clef-pattern pairing is the current stand-in); per-staff multi-voice. | piano target reached. |
