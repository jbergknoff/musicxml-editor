...
Status: **Phases 0–3 are done** (see `AGENTS.md` for the as-built notes).
...
| **4 — Full single staff** ✅ | ~~key/time sig~~ (clef/key/time recovered by `lib/transcription/decode-attributes.ts`, incl. mid-staff changes), ~~beams~~ (inferred per beat by `lib/assembly/beams.ts`), ~~multi-page~~ (stitched by `lib/assembly/combine-pages.ts`), ~~grand staff paired into a two-staff part~~ (`lib/staves/system-grouping.ts` + `buildScore`). | assembly fidelity. |
| **5 — Grand staff** | ~~paired staves, two-staff part, per-staff voices~~ (done — `buildScore` emits `<staff>`/`<backup>`/per-staff voices) and ~~brace detection from the image~~ (done — `detectBraces` scans the left margin for ink bridging the inter-staff gap; `groupSystems` groups brace-linked staves, with the clef pattern as fallback). **Remaining:** per-staff multi-voice. | piano target reached. |
