...
// Every staff's slot whose onset exactly matches a specific (measureIndex,
// onsetBeat) position — one per part that has an onset there, omitted for a
// staff with no onset at that exact beat (mid-note or mid-rest on that staff).
// This is the single source of truth for "what belongs to this selection on
// other staves": the inspector's per-staff rows and the score's cross-staff
// highlight both read the same onset-exact set, so what's listed is exactly
// what's tinted.
